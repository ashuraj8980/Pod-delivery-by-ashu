...
export default function PODTool() {
  ...
  const [selectedReplacerIds, setSelectedReplacerIds] = useState<Set<string>>(new Set());
    ...
    reader.onload = (evt) => {
      setTimeout(() => {
        try {
          ...
          if (parsedRows.length === 0) throw new Error("No valid shipments found. Ensure AWB and Status columns exist.");
    ...
    reader.onload = (evt) => {
      setTimeout(() => {
        try {
          ...
          // Ultra-Flexible Regex Detection
          const remarkKey = headers.find(h => /remark|nsl|statuscomment|comment|reason|remarks/i.test(h)) || "";
          const awbKey = headers.find(h => /awb|waybill|waybillno|awbno|no/i.test(h)) || "";
          ...
          setReplacerMeta({ headers, remarkKey, awbKey });
          setSelectedReplacerIds(new Set());
  ...
  return (
    <div className="min-h-screen bg-[#F0F4FA] font-body text-[#374151]">
      <div className="fixed top-0 left-0 w-full z-[300] shadow-lg">
        <div className="h-[3px] w-full bg-gradient-to-r from-[#1565C0] via-[#F9A825] via-[#2E7D32] to-[#D32F2F]" />
        <header className="h-[58px] bg-[#1C2333] px-6 flex items-center justify-between text-white">
          ...
          <div className="flex items-center gap-4">
            ...
            {currentSession && <div className="bg-[#F9A825]/20 px-3 py-1 rounded-full text-[9px] font-mono font-black text-[#F9A825] uppercase">{currentSession.data.length} ROWS</div>}
          </div>
        </header>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-[14px] p-6 shadow-sm border border-[#E2E8F0]">
              ...
              <div className="flex flex-col md:flex-row gap-4 mb-6">
                <input type="number" value={setupData.dspId} onChange={(e) => setSetupData({...setupData, dspId: e.target.value})} className="flex-1 bg-slate-50 border rounded-xl px-4 py-2.5 text-[13px] font-bold outline-none" placeholder="DSP ID" />
                <input type="text" value={setupData.feName} onChange={(e) => setSetupData({...setupData, feName: e.target.value})} className="flex-1 bg-slate-50 border rounded-xl px-4 py-2.5 text-[13px] font-bold outline-none" placeholder="FE Name" />
              ...
              <div className={cn("border-2 border-dashed rounded-2xl p-10 text-center transition-all cursor-pointer relative", uploadError ? "border-red-400 bg-red-50" : "border-[#CBD5E1] bg-[#F8FAFC] hover:border-primary", isProcessing && "opacity-80 pointer-events-none")}>
            ...
            {sessions.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {sessions.map(s => (
                  <div key={s.id} onClick={() => setSelectedSessionId(s.id)} className={cn("bg-white p-4 rounded-xl border-l-[4px] shadow-sm cursor-pointer relative transition-all", selectedSessionId === s.id ? "border-l-[#1565C0] bg-[#F0F7FF] ring-1 ring-[#1565C0]/20" : "border-l-slate-300 border border-slate-200")}>
                    <button onClick={(e) => { e.stopPropagation(); setSessions(prev => prev.filter(x => x.id !== s.id)); if(selectedSessionId === s.id) setSelectedSessionId(null); }} className="absolute top-3 right-3 text-slate-300 hover:text-red-500"><X className="w-4 h-4" /></button>
              ...
              <div className="space-y-6">
                  ...
                  <div className="bg-white rounded-[14px] p-6 shadow-sm border border-slate-200 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {pendingRemarkStats.map((stat, i) => (
                        <button key={i} onClick={() => setActiveRemarkChip(activeRemarkChip === stat.text ? null : stat.text)} className={cn("flex items-center gap-2 px-3 py-1.5 rounded-lg border text-[10px] font-bold uppercase transition-all", activeRemarkChip === stat.text ? "bg-[#1565C0] text-white border-[#1565C0]" : "bg-[#F3F4F6] border-[#E5E7EB] text-[#374151] hover:bg-slate-200")}>
                          <span>{stat.text || "No Remark"}</span>
                          <span className="bg-white text-slate-500 px-1.5 rounded-full text-[9px] shadow-sm">{stat.count}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-2">
                    <button onClick={downloadExcel} className="bg-[#388E3C] hover:bg-[#2E7D32] text-white px-5 py-2.5 rounded-lg font-[700] text-[12px] uppercase tracking-widest flex items-center gap-2 shadow-sm"><Download className="w-4 h-4" /> Download Excel</button>
                    <button onClick={copyTable} className="bg-[#1976D2] hover:bg-[#1565C0] text-white px-5 py-2.5 rounded-lg font-[700] text-[12px] uppercase tracking-widest flex items-center gap-2 shadow-sm"><Copy className="w-4 h-4" /> Copy Table</button>
                  </div>
                  <button onClick={() => { setSessions(prev => prev.map(s => s.id === selectedSessionId ? {...s, data: []} : s)); }} className="text-[11px] font-black text-slate-400 hover:text-red-500 uppercase tracking-widest px-4">Clear Session</button>
                </div>

                <div className="bg-white rounded-[14px] shadow-sm border border-slate-200 overflow-hidden">
                  <div className="p-4 border-b bg-slate-50 flex items-center justify-between">
                    <div className="flex items-center gap-2"><span className="text-[11px] font-black uppercase">{statusFilter.toUpperCase()} SHIPMENTS</span><span className="px-2 py-0.5 bg-[#CBD5E1] rounded text-[9px] font-black uppercase">{filteredRows.length} ROWS</span></div>
                    <div className="relative"><Search className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="Search AWB..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-white border rounded-full pl-9 pr-4 py-1.5 text-[11px] font-bold outline-none w-[220px]" /></div>
                  </div>
                  <div className="overflow-x-auto max-h-[700px] custom-scrollbar">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-[#1C2333] text-white">
                          <th className="p-4 w-[40px] text-center"><input type="checkbox" className="accent-[#F9A825]" /></th>
                          <th className="p-4 w-[40px] text-center">X</th>
                          <th className="p-4 w-[80px] text-[9.5px] uppercase font-[700]">DSP ID</th>
                          <th className="p-4 w-[160px] text-[9.5px] uppercase font-[700]">AWB Number</th>
                          <th className="p-4 w-[140px] text-[9.5px] uppercase font-[700]">Client</th>
                          <th className="p-4 w-[140px] text-[9.5px] uppercase font-[700]">Order ID</th>
                          <th className="p-4 text-[9.5px] uppercase font-[700]">Remark</th>
                          <th className="p-4 w-[120px] text-[9.5px] uppercase font-[700]">FE Name</th>
                        </tr>
                      </thead>
                      <tbody>
                        {statusFilter === 'pending' && groupedPendingRows ? (
                          groupedPendingRows.map(([remark, rows]) => (
                            <React.Fragment key={remark}>
                              <tr className="bg-gradient-to-r from-[#0D1B2E] to-[#1A2F4A] border-y border-white/5">
                                <td colSpan={8} className="p-3 px-5"><div className="flex items-center gap-3"><Star className="w-3.5 h-3.5 text-[#F9A825] fill-current" /><span className="text-[10px] font-black text-white uppercase tracking-[0.1em]">{remark}</span><span className="px-2 py-0.5 bg-[#F9A825] text-white rounded text-[9px] font-black uppercase">{rows.length} PKT</span></div></td>
                              </tr>
                          ...
                          <>
                            {currentSession && (
                              <tr className="bg-gradient-to-r from-[#0D1B2E] to-[#1A2F4A] border-y border-white/5">
                                <td colSpan={8} className="p-3 px-5"><div className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="text-[14px] font-mono font-black text-[#F9A825]">{currentSession.dspId}</span><span className="px-2 py-0.5 bg-[#F9A825] text-white rounded text-[9px] font-black uppercase">{filteredRows.length} PKT</span></div><div className="text-[#6B8CAE] text-[10px] font-bold uppercase tracking-widest">{currentSession.feName} · {currentSession.date}</div></div></td>
                              </tr>
                            )}
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-white rounded-[14px] p-6 shadow-sm border border-slate-200">
                ...
                <div className={cn("border-2 border-dashed rounded-2xl p-10 text-center transition-all cursor-pointer relative", uploadError ? "border-red-400 bg-red-50" : "border-slate-200 bg-[#F8FAFC] hover:border-primary", isProcessing && "opacity-80 pointer-events-none")}>
                  ...
                  {isProcessing ? (
                    <div className="flex flex-col items-center gap-3"><Loader2 className="w-10 h-10 text-primary animate-spin" /><p className="text-[11px] font-black uppercase text-primary tracking-widest">ANALYZING FILE...</p></div>
                  ) : uploadError ? (
                    <div className="space-y-3"><AlertCircle className="w-10 h-10 text-red-500 mx-auto" /><p className="text-[14px] font-[700] text-red-600 uppercase">Invalid File Format</p><p className="text-[10px] text-red-400 font-bold uppercase leading-relaxed">{uploadError}</p><button onClick={() => setUploadError(null)} className="bg-red-600 text-white px-4 py-1.5 rounded-lg text-[10px] font-black uppercase mt-4">Try Again</button></div>
                  ) : (
                    <div className="space-y-3"><Download className="w-10 h-10 text-primary/40 mx-auto" /><p className="text-[15px] font-[700] text-[#111827]">Drop Any Excel/CSV Sheet here, or click to upload</p><p className="text-[11px] text-[#64748B] uppercase tracking-widest">Preserves all original columns and order</p></div>
                  )}
                </div>
              </div>
              ...
              {replacerData.length > 0 && (
                <div className="bg-white rounded-[14px] shadow-sm border border-slate-200 overflow-hidden animate-in slide-in-from-bottom-5">
                  <div className="p-4 bg-slate-50 border-b flex items-center justify-between">
                    <div className="flex gap-2">
                      <button onClick={() => {
                        const cleanData = replacerData.filter(r => selectedReplacerIds.size === 0 || selectedReplacerIds.has(r.__id)).map(r => {
                          const { __isReplaced, __id, ...rest } = r;
                          const awbK = replacerMeta?.awbKey || "";
                          if (awbK) rest[awbK] = { v: String(rest[awbK]), t: 's', z: '@' };
                          return rest;
                        });
                      ...
                      }} className="bg-[#2E7D32] hover:bg-[#1B5E20] text-white px-6 py-2.5 rounded-lg font-[700] text-[12px] uppercase tracking-widest flex items-center gap-2 shadow-sm transition-all"><Download className="w-4 h-4" /> Download Official Excel</button>
                      
                      <button onClick={() => {
                        const rowsToCopy = replacerData.filter(r => selectedReplacerIds.size === 0 || selectedReplacerIds.has(r.__id));
                        const headers = replacerMeta?.headers.join('\t') || "";
                        const rows = rowsToCopy.map(r => {
                          const { __isReplaced, __id, ...rest } = r;
                          return Object.values(rest).join('\t');
                        }).join('\n');
                        navigator.clipboard.writeText(headers + '\n' + rows).then(() => showToast("Copied table to clipboard", "ok"));
                      }} className="bg-[#1565C0] hover:bg-[#0D47A1] text-white px-6 py-2.5 rounded-lg font-[700] text-[12px] uppercase tracking-widest flex items-center gap-2 shadow-sm transition-all"><Copy className="w-4 h-4" /> Copy Full Table</button>
                    </div>
                  ...
                  <div className="overflow-x-auto max-h-[600px] custom-scrollbar">
                    <table className="w-full text-left border-collapse text-[11px]">
                      <thead className="sticky top-0 z-30">
                        <tr className="bg-[#1C2333] text-white">
                          <th className="p-4 w-[40px] text-center"><input type="checkbox" onChange={(e) => {
                            if (e.target.checked) setSelectedReplacerIds(new Set(replacerData.map(r => r.__id)));
                            else setSelectedReplacerIds(new Set());
                          }} checked={selectedReplacerIds.size === replacerData.length} /></th>
                          <th className="p-4 w-[40px] text-center">X</th>
                      ...
                      <tbody>
                        {replacerData.map((row, idx) => (
                          <tr key={row.__id} className={cn("border-b transition-colors", row.__isReplaced ? "bg-[#F0FDF4]" : "bg-[#FFFDE7]", selectedReplacerIds.has(row.__id) && "bg-blue-100")}>
                            <td className="p-3 text-center"><input type="checkbox" checked={selectedReplacerIds.has(row.__id)} onChange={() => {
                              const newSet = new Set(selectedReplacerIds);
                              if (newSet.has(row.__id)) newSet.delete(row.__id);
                              else newSet.add(row.__id);
                              setSelectedReplacerIds(newSet);
                            }} /></td>
                            <td className="p-3 text-center"><button onClick={() => setReplacerData(prev => prev.filter(r => r.__id !== row.__id))} className="text-slate-300 hover:text-red-500"><Trash2 className="w-3.5 h-3.5 mx-auto" /></button></td>
                            {replacerMeta?.headers.map((h, i) => (
                              <td key={i} className={cn("p-3 whitespace-nowrap", h === replacerMeta.remarkKey && row.__isReplaced ? "text-[#2E7D32] font-black" : h === replacerMeta.remarkKey ? "text-[#D97706] font-black" : "text-slate-600")}>
                                {row[h] instanceof Date ? row[h].toLocaleDateString() : row[h]}
                              </td>
                            ))}
                          </tr>
                        ))}
            ...
            <div className="lg:col-span-4">
              <div className="bg-white rounded-[14px] shadow-sm border border-slate-200 overflow-hidden sticky top-[130px]">
                <div className="bg-[#2E7D32] p-4 text-white shadow-md">
                  <h3 className="text-[11px] font-[800] uppercase tracking-widest flex items-center gap-2">REMARK MAPPING</h3>
                ...
                <div className="p-4 overflow-y-auto max-h-[70vh] space-y-3 custom-scrollbar">
                  {Object.entries(REMARK_MAPPING).map(([nsl, official]) => (
                    <div key={nsl} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-3 hover:bg-white shadow-sm">
                      ...
                      <div className="flex justify-center"><ArrowRight className="w-3.5 h-3.5 text-green-600" /></div>
...
function DataRow({ row, idx, isFirstInGroup, onDelete }: { row: PODRow, idx: number, isFirstInGroup: boolean, onDelete: (id: string) => void }) {
  return (
    <tr className={cn("border-b transition-colors group", row.isIntact ? "bg-[#FFF5F5]" : "hover:bg-[#F0F7FF]")}>
      ...
      <td className="p-3 text-center"><button onClick={() => onDelete(row.id)} className="text-slate-300 hover:text-red-500"><Trash2 className="w-3.5 h-3.5 mx-auto" /></button></td>
      ...
      <td className="p-3 text-[11px] text-[#374151] truncate max-w-[140px]">{row.client}</td>
      <td className="p-3 text-[11px] text-[#374151] truncate max-w-[140px]">{row.orderId}</td>
      <td className="p-3">
        <span className={cn("px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-tight border shadow-sm", row.isIntact ? "bg-[#FFF5F5] text-[#D32F2F] border-[#FFCDD2]" : "bg-[#FFFDE7] text-[#D97706] border-[#FDE68A]")}>
          {row.remark || "N/A"}
        </span>
      </td>
