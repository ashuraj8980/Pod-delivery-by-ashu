...
/**
 * @fileOverview Delhivery POD Management Tool v4.0
 * Features: Compact UI, Error Handling, Session Persistence, Duplicate Highlighting.
 */
...
export default function PODTool() {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (parsedRows.length > 0) {
          ...
          showToast("No valid Delhivery rows found!", "err");
        }
      } catch (err) {
        showToast("Error parsing file! Please upload a valid Delhivery export.", "err");
      }
    };
    reader.onerror = () => showToast("FileReader Error!", "err");
  ...
  const handleReplacerUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (!rawData || rawData.length === 0) throw new Error("Empty file");
  ...
  return (
    <div className="min-h-screen bg-[#F0F4FA] font-body text-[#1C2333] select-auto overflow-x-hidden">
      ...
      <header className="h-[55px] bg-[#1C2333] px-6 flex items-center justify-between text-white shadow-lg relative z-[90]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="text-[14px] font-black tracking-tight leading-none">POD Management Tool</h1>
      ...
      <main className="p-4 md:p-6 max-w-[1300px] mx-auto space-y-6">
        {activeTab === "eod" ? (
          <div className="space-y-6">
            <div className="bg-white rounded-[1.5rem] p-6 shadow-md border border-slate-200">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[13px] font-code font-bold outline-none focus:border-[#1565C0] transition-all" 
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-[13px] font-bold outline-none focus:border-[#1565C0] transition-all" 
              ...
              <div 
                className={cn(
                  "border-2 border-dashed rounded-[1.5rem] p-8 text-center transition-all relative group overflow-hidden",
              ...
              >
                ...
                <div className="space-y-3">
                  <Download className={cn("w-8 h-8 mx-auto transition-all", (!setupData.feName || !setupData.dspId) ? "text-slate-300" : "text-blue-500")} />
                  <div>
                    <p className="text-[12px] font-black text-slate-700 uppercase tracking-widest">
                      {(!setupData.feName || !setupData.dspId) ? "Required: Fill DSP & FE Name" : "Upload Delhivery Export File"}
                    </p>
                    <p className="text-[9px] text-slate-400 font-bold uppercase mt-1 tracking-widest">Excel & CSV Only</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {sessions.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center justify-between px-1">
                  <h2 className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em] flex items-center gap-2"><Layers className="w-4 h-4" /> Database</h2>
                  <button onClick={() => { if(confirm("Clear all?")) setSessions([]); }} className="text-[9px] font-black text-red-600 hover:text-red-700 uppercase tracking-widest flex items-center gap-1 border-b border-red-200"><Trash2 className="w-3 h-3" /> Clear All</button>
                </div>
                  ...
                  {sessions.map(s => (
                    <div 
                      ...
                      className={cn(
                        ...
                        selectedSessionId === s.id ? "border-blue-500 ring-2 ring-blue-500/10 shadow-md" : "border-slate-200 hover:border-blue-300"
                      )}
                    >
                      <button 
                      ...
                      <p className="text-[11px] font-black truncate text-slate-800">{s.feName}</p>
                      <p className="text-[9px] font-code text-slate-400 font-bold">DSP: {s.dspId}</p>
                      <div className="mt-2 text-[12px] font-black text-blue-600">{s.data.length} <span className="text-[8px] text-slate-400">PKTS</span></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-[1.5rem] shadow-xl border border-slate-200 overflow-hidden">
                <div className="p-4 bg-slate-50 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
                  <div className="flex gap-2">
                    <button onClick={() => downloadExcel(filteredRows, currentSession)} className="bg-[#2E7D32] text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 shadow-sm"><Download className="w-4 h-4" /> Export</button>
                    <button onClick={() => copyTable(filteredRows)} className="bg-[#1565C0] text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 shadow-sm"><Copy className="w-4 h-4" /> Copy</button>
                  </div>
                  <div className="flex items-center gap-2">
                    {[
                      ...
                      <button 
                        ...
                        className={cn(
                          "px-4 py-2 rounded-lg border transition-all text-center min-w-[50px]",
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 z-30">
                      <tr className="bg-[#1C2333] text-white text-[9px] font-black uppercase tracking-widest">
                        <th className="p-4 w-[50px] text-center">Action</th>
                        <th className="p-4 w-[100px]">DSP ID</th>
                        <th className="p-4 w-[160px]">AWB Number</th>
                        <th className="p-4 w-[140px]">Client</th>
                        <th className="p-4 w-[140px]">Order ID</th>
                        <th className="p-4">Remark</th>
                        <th className="p-4 w-[120px]">FE Name</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredRows.map((row, idx) => {
                        ...
                        return (
                          <tr key={row.id} className={cn(
                            ...
                            <td className="p-3 font-code text-[11px] font-bold text-slate-400">{idx === 0 ? row.dspId : ""}</td>
                            <td 
                              className={cn(
                                "p-3 font-code text-[12px] font-black tracking-wider cursor-pointer hover:underline flex items-center gap-2",
                            ...
                            <td className="p-3 text-[11px] font-bold text-slate-700 truncate max-w-[140px]">{row.client}</td>
                            <td className="p-3 text-[11px] text-slate-500 font-medium truncate max-w-[140px]">{row.orderId}</td>
                            <td className="p-3">
                              <span className={cn(
                            ...
                            <td className="p-3 text-[11px] font-black text-slate-500 truncate">{row.feName}</td>
                          </tr>
                        );
                      })}
                    </tbody>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-[1.5rem] p-6 border border-slate-200 shadow-lg space-y-6">
                <h2 className="text-xl font-black tracking-tighter flex items-center gap-2"><FileSpreadsheet className="w-6 h-6 text-green-600" /> Remark Replacer</h2>
                <div className="border-2 border-dashed border-green-50 rounded-2xl p-10 text-center hover:border-green-300 hover:bg-green-50/20 transition-all cursor-pointer relative group overflow-hidden">
                  ...
                  <Download className="w-10 h-10 text-green-200 mx-auto transition-transform group-hover:scale-110" />
                  <div className="mt-4">
                    <p className="text-[12px] font-black text-slate-700 uppercase tracking-widest italic">Upload EOD Export</p>
                    <p className="text-[8px] font-bold text-slate-400 mt-1 tracking-widest uppercase">Excel & CSV Only</p>
                  </div>
                </div>

                {replacerData.length > 0 && (
                  <div className="grid grid-cols-3 gap-3 pt-2">
                    <div className="bg-slate-900 p-4 rounded-xl text-center shadow">
                    ...
                    <div className="bg-green-600 p-4 rounded-xl text-center shadow">
                       ...
                       <p className="text-[8px] font-black text-white/70 uppercase tracking-widest">Fixed</p>
                    </div>
                    <div className="bg-amber-500 p-4 rounded-xl text-center shadow">
                       <p className="text-xl font-black text-white">{replacerStats.missing}</p>
                       <p className="text-[8px] font-black text-white/70 uppercase tracking-widest">N/A</p>
                    </div>
                  </div>
                )}
              </div>

              <div className="bg-[#1C2333] rounded-[1.5rem] p-6 text-white shadow-xl relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-[4px] bg-gradient-to-r from-blue-500 via-green-500 to-amber-500" />
                <h3 className="text-[10px] font-black text-[#F9A825] uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
                  <AlertCircle className="w-4 h-4" /> Official Guide
                </h3>
                <div className="overflow-y-auto max-h-[300px] space-y-2 custom-scrollbar pr-3">
                  {Object.entries(REMARK_MAPPING).map(([old, official]) => (
                    <div key={old} className="p-3 bg-white/5 rounded-xl text-[9px] border border-white/10">
                      <p className="text-slate-500 italic font-bold mb-1 truncate">{old}</p>
                      <p className="text-green-400 font-black flex items-center gap-2 text-[11px]">
                        <CheckCircle2 className="w-3.5 h-3.5 text-green-500" /> {official}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white rounded-[1.5rem] shadow-xl border border-slate-200 overflow-hidden">
                <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                  <div className="flex gap-2">
                    ...
                    }} className="bg-[#2E7D32] text-white px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 shadow-sm"><Download className="w-4 h-4" /> Download</button>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead className="sticky top-0 z-30">
                      <tr className="bg-[#1C2333] text-white text-[9px] font-black uppercase tracking-widest">
                        ...
                        <th className="p-4 w-[140px]">Original</th>
                        <th className="p-4 w-[120px]">FE Name</th>
                      </tr>
                    </thead>
                    <tbody>
                      {replacerData.map((row, idx) => (
                        <tr key={idx} className={cn(
                          ...
                          row.isReplaced ? "bg-green-50/10" : "bg-amber-50/10"
                        )}>
                          <td className="p-3 text-[10px] font-black text-slate-400">{row.id}</td>
                          <td className="p-3 text-[10px] text-slate-600 font-bold">{row.date}</td>
                          <td 
                            className="p-3 font-code text-[12px] font-black text-[#1565C0] cursor-pointer hover:underline"
                            onClick={() => copyToClipboard(row.awb, "AWB Copied")}
                          >
                            {row.awb}
                          </td>
                          ...
                          <td className="p-3 text-[9px] text-slate-400 italic font-bold truncate max-w-[140px]">{row.oldRemark}</td>
                          <td className="p-3 text-[10px] font-black text-slate-700 truncate">{row.feName}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <style jsx global>{`
        ...
        body { font-family: 'Inter', sans-serif; overflow-x: hidden; background: #F0F4FA; }
