...
export default function PODTool() {
  ...
  const handleOTPFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const activeAWBMap = new Map<string, { addr: string, status: string }>();
        currentSession?.data.forEach(row => {
          if (row.awb) activeAWBMap.set(row.awb.trim(), { addr: row.returnAddress || "", status: row.status });
        });

        const matched: OTPRow[] = [];
        const matchedAWBsInOTP = new Set<string>();

        // 1. Process OTP file rows
        rawData.forEach((row: any) => {
          ...
          const sessionInfo = activeAWBMap.get(awb);
          
          if (sessionInfo) {
            matchedAWBsInOTP.add(awb);
            ...
            // Logic Cases for UI/Highlights
            if (otpStatus === 'dispatched' && sessionInfo.status === 'dispatched') logicCase = 1;
            else if (otpStatus === 'dispatched' && sessionInfo.status === 'pending') logicCase = 2;
            else if (otpStatus === 'pending' && sessionInfo.status === 'pending') logicCase = 3;
            else if (otpStatus === 'rto') logicCase = 4;
            else if (otpStatus === 'dto') logicCase = 5;
            else if (otpStatus === 'pending' && sessionInfo.status === 'dispatched') logicCase = 6;

            if (otpStatus !== 'unknown') {
              matched.push({
                id: crypto.randomUUID(),
                awb,
                client,
                ...
                returnAddress: sessionInfo.addr,
        ...
        // 2. Add remaining Pending rows from session that were NOT in OTP
        currentSession?.data.forEach(row => {
          if (row.status === 'pending' && !matchedAWBsInOTP.has(row.awb)) {
            matched.push({
              id: crypto.randomUUID(),
              awb: row.awb,
              client: row.client,
        ...
        if (matched.length === 0) showToast("No matching AWBs found between OTP file and current session.", "err");
        else showToast(`Matched ${matched.length} rows with current session.`, "ok");
  ...
  const handleOtpDownload = () => {
    ...
    const data = otpFilteredRows.map(r => [
      { v: r.awb, t: 's' },
      r.client,
      r.otpStatus === 'not_found' ? 'NOT FOUND' : r.otpStatus.toUpperCase(),
  ...
  const otpFilteredRows = useMemo(() => {
    ...
    } else if (otpStatusFilter !== 'all') {
      rows = rows.filter(r => r.otpStatus === otpStatusFilter);
    }
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-center space-y-6">
              <div className="flex flex-col items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-medium">Match Delhivery OTP Export with Session CSV data for discrepancies.</p>
                </div>
              </div>
              ...
              <div className={cn("border-2 border-dashed rounded-xl p-10 transition-all cursor-pointer relative bg-slate-50 hover:bg-white hover:border-blue-500 max-w-2xl mx-auto", isProcessing && "opacity-50")}>
                <input type="file" onChange={handleOTPFileUpload} disabled={isProcessing || !selectedSessionId} className="absolute inset-0 opacity-0 cursor-pointer z-10" />
                <div className="space-y-3">
                  <Download className="w-8 h-8 text-slate-300 mx-auto" />
                  <p className="text-sm font-black text-[#111827]">Upload Delhivery OTP Report</p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={async () => {
                      const headers = otpStatusFilter === 'pending' 
                        ? ['AWB Number', 'Client Name', 'OTP Status', 'CSV Status', 'Return Address']
                        : ['AWB Number', 'Client Name', 'Current Status', 'Return Address'];
                      const data = otpFilteredRows.map(r => {
                        if (otpStatusFilter === 'pending') {
                          return {
                            'AWB Number': r.awb,
                            'Client Name': r.client,
                            'OTP Status': r.otpStatus === 'not_found' ? 'NOT FOUND' : r.otpStatus.toUpperCase(),
                            'CSV Status': 'PENDING',
                    ...
                    <table className="w-full text-center border-collapse bg-white table-fixed">
                        ...
                        <tr className="h-11">
                          ...
                          {otpStatusFilter === 'pending' ? (
                            <>
                              ...
                              <th style={{ minWidth: '350px' }} className="px-4 text-[11px] font-bold text-center">Return Address</th>
                            </>
                          ) : (
                            <>
                              <th style={{ width: '150px' }} className="px-4 text-[11px] font-bold text-center">AWB Number</th>
                      ...
                      <tbody>
                        {otpFilteredRows.length > 0 ? (
                                ...
                                {groupRows.map((row) => (
                                  <tr key={row.id} 
                                    className={cn(
                                      ...
                                      row.isFTPL && row.otpStatus !== 'dispatched' ? "bg-[#F0FDF4]" : ""
                                    )}
                                  >
                                    ...
                                    <td className={cn("px-4 text-[13px] font-semibold truncate", 
                                      ...
                                      row.isFTPL && row.otpStatus !== 'dispatched' ? "text-[#16A34A] font-bold" : "text-[#1565C0]")}>
                                      {row.client}
                                    </td>
                                    <td className="px-4">
                                      <div className="flex flex-col items-center gap-1">
                                        <span className={cn(
                                          ...
                                          row.otpStatus === 'not_found' ? "bg-slate-100 text-slate-400 border-slate-200" :
                                          "bg-[#16A34A] text-white border-[#16A34A]"
                                        )}>
                                          {row.otpStatus === 'not_found' ? 'NOT FOUND' : row.otpStatus}
                                        </span>
                                        {row.logicCase === 2 && (
                                        ...
                                        {row.logicCase === 6 && (
                                          <span className="bg-rose-100 text-rose-700 text-[9px] font-black px-1.5 py-0.5 rounded border border-rose-200 uppercase tracking-tighter">
                                            Dispatched in CSV
                                          </span>
                                        )}
                                      </div>
                                    </td>
                                    <td className="px-4 py-2 text-[12px] font-medium text-slate-500 text-center" style={{ whiteSpace: 'normal', wordWrap: 'break-word' }}>
                                      {row.returnAddress || "—"}
                                    </td>
                                  </tr>
                                ))}
                          ...
                          <tr><td colSpan={otpStatusFilter === 'pending' ? 6 : 5} className="h-32 text-center text-slate-300 font-bold uppercase tracking-widest">No matching AWBs found between OTP file and current session.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
