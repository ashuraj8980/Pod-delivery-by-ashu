...
// Professional Advanced Multi-Select Filter Component
const MultiSelectFilter = ({ 
  ...
  const isAllVisibleSelected = useMemo(() => {
    return filteredOptions.length > 0 && filteredOptions.every(([client]) => selected.includes(client));
  }, [filteredOptions, selected]);

  const handleToggleAllVisible = () => {
    const allAlreadySelected = filteredOptions.every(([client]) => selected.includes(client));
    filteredOptions.forEach(([client]) => {
      const isCurrentlySelected = selected.includes(client);
      if (allAlreadySelected) {
        if (isCurrentlySelected) onToggle(client);
      } else {
        if (!isCurrentlySelected) onToggle(client);
      }
    });
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <Popover onOpenChange={(open) => { if (!open) setSearch(""); }}>
        <PopoverTrigger asChild>
          <button className="flex items-center justify-between border border-slate-200 rounded-lg px-3 py-2 text-[13px] font-bold bg-white outline-none focus:border-blue-500 w-[240px] shadow-sm hover:bg-slate-50 transition-colors group">
            <span className="truncate flex-1 text-left">
              {selected.length === 0 ? "All Clients" : `${selected.length} Client(s) Selected`}
            </span>
            <div className="flex items-center gap-2">
              {selected.length > 0 && (
                <span className="w-5 h-5 bg-blue-600 text-white text-[10px] flex items-center justify-center rounded-full font-black animate-in zoom-in-50 duration-200">
                  {selected.length}
                </span>
              )}
              <ChevronDown className="w-4 h-4 text-slate-400 shrink-0 group-hover:text-slate-600 transition-transform" />
            </div>
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[300px] p-0" align="start">
          <div className="p-3 border-b space-y-3 bg-white">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Client Selection</span>
              <button onClick={onClear} className="text-[10px] font-bold text-blue-600 hover:text-blue-800 transition-colors uppercase">Clear Filters</button>
            </div>
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-md pl-8 pr-3 h-9 text-[12px] font-bold outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
              />
            </div>
          </div>
            ...
            {filteredOptions.length > 0 ? (
              <div className="space-y-0.5">
                <div 
                  ...
                  className="flex items-center gap-3 px-3 py-2 hover:bg-blue-50/50 rounded-md cursor-pointer transition-all group"
                >
                  <Checkbox checked={isAllVisibleSelected} />
                  <span className="text-[13px] font-black text-slate-800">Select All Visible</span>
                </div>
                <div className="h-px bg-slate-100 my-1" />
                {filteredOptions.map(([client, count]) => (
                  <div 
                    key={client} 
                    onClick={() => onToggle(client)}
                    className={cn(
                      "flex items-center gap-3 px-3 py-2 rounded-md cursor-pointer transition-all group",
                      selected.includes(client) ? "bg-blue-50/30" : "hover:bg-slate-50"
                    )}
                  >
                    ...
                    <div className="flex-1 flex items-center justify-between min-w-0">
                      <span className={cn(
                        ...
                        selected.includes(client) ? "text-blue-700" : "text-slate-700"
                      )}>{client}</span>
                      <span className={cn(
                        "text-[10px] font-black px-1.5 py-0.5 rounded ml-2 shrink-0 transition-all",
                        selected.includes(client) ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500 group-hover:bg-slate-200"
                      )}>{count}</span>
                    </div>
                  </div>
                ))}
              ...
              <div className="py-12 px-4 text-center">
                ...
                <p className="text-[12px] font-bold text-slate-400">No matching clients</p>
              </div>
            )}
          </div>
        </PopoverContent>
...
function PODToolContent() {
  ...
  const handleCopyTable = useCallback(async (rowsToCopy: any[]) => {
    ...
    const isOtpTab = activeTab === 'otp';
    let htmlTable = '<table border="1"><tbody>';
    
    if (isOtpTab) {
      htmlTable += rowsToCopy.map((r) => `
        <tr>
          <td style="mso-number-format:'\\@'">${normalizeAWB(r.awb)}</td>
          <td>${r.client}</td>
          <td>${r.otpStatus}</td>
          <td>${r.sessionStatus}</td>
          <td>${r.returnAddress}</td>
        </tr>
      `).join('');
    } else {
      htmlTable += rowsToCopy.map((r, i) => `
        <tr>
  ...
  }, [showToast, currentSession, activeTab]);
  ...
  const downloadExcel = (rowsToDownload: any[]) => {
    ...
    if (isOtpTab) {
      header = ['Waybill Number', 'Client', 'OTP Status', 'Session Status', 'Address'];
      excelData = rowsToDownload.map((r) => [
      ...
      header = ['Date', 'DSP ID', 'Waybill Number', 'Client', 'Order ID', 'Remark', 'Amount', 'FE Name'];
      excelData = rowsToDownload.map((r, i) => [
        formatDate(currentSession?.date), 
        ...
        <div className="flex gap-8 h-full">
          {['eod', 'remark', 'otp'].map(id => (
            <button key={id} onClick={() => {
              ...
              if (id === 'otp') setOtpData([]);
            }} className={cn("h-full px-1 text-[13px] font-semibold transition-all relative border-b-2", activeTab === id ? "text-white border-blue-500" : "text-slate-400 border-transparent hover:text-white")}>
                ...
                <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
                  {sessions.map(s => (
                    <div key={s.id} onClick={() => handleSessionClick(s)} className={cn("relative p-4 border-[1.5px] rounded-2xl cursor-pointer transition-all shadow-sm overflow-hidden bg-white", selectedSessionId === s.id ? "border-blue-500 ring-2 ring-blue-500/10" : "hover:border-blue-300 border-slate-100")}>
                      ...
                      <p className="text-[11px] text-slate-400 font-black mb-3 uppercase tracking-wider">{s.dspId} — {s.date} — {s.time || ""}</p>
                          ...
                          <React.Fragment key={`group-frag-${client}`}>
                            ...
                            {rows.map((row: any) => (
                              <tr key={`row-${row.id}`} className={cn("border-b hover:bg-blue-50/40", selectedRowIds.has(row.id) && "bg-blue-50/50", row.status === 'Pending' && (row.amount ?? 0) >= 4000 && "bg-rose-50/30")}>
                                ...
                                <td className="px-2 py-2 text-[13px] font-black font-mono text-blue-700 cursor-pointer hover:underline" onClick={() => { navigator.clipboard.writeText(normalizeAWB(row.awb)); showToast("Waybill Copied", "ok"); }}>{normalizeAWB(row.awb)}</td>
